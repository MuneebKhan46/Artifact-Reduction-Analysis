...
class ImageDataset(Dataset):
    def __init__(self, csv_path, original_dir, denoised_dir, patch_size=224):
        self.original_patches, self.denoised_patches, self.scores, self.denoised_image_names, self.patch_numbers = load_data_from_csv(csv_path, original_dir, denoised_dir)
        self.pairs = []
        self.n0_count = 0
        self.non_n0_count = 0

        for i in range(len(self.scores)):
            if self.scores[i] == 1:  # Keep only patches with label 1
