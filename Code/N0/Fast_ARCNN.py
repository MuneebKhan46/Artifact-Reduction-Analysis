...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, 'HighFreq_FastARCNN_Model(N0).pth'))
        print(f"New best model saved with validation loss: {val_loss:.4f}")




model.load_state_dict(torch.load(os.path.join(model_dir, 'HighFreq_FastARCNN_Model(N0).pth')))
model.eval()
...
image_save_dir = os.path.join(Results_dir, 'images/FastAR-CNN(N0)/')
...
results.append({'Model': 'Fast AR-CNN', 'Image Type': 'RGB', 'Noise Type': 'N0', 'PSNR': avg_psnr, 'SSIM': avg_ssim})
