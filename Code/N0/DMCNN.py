...
def load_data_from_csv(csv_path, original_dir, denoised_dir):
    ...
    for _, row in df.iterrows():
        ...
        original_file_name = f"original_{image_name}.png"
        denoised_file_name = f"denoised_{image_name}.png"
...
class CustomDataset(Dataset):
    def __init__(self, original_dir, denoised_dir, csv_path, transform=None):
        ...
        # Load data with filtering for 'n0' files
...
print(f"Total images with 'n0': {len(dataset)}")
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, 'HighFreq_DMCNN_model(N0).pth'))
...
model.load_state_dict(torch.load(os.path.join(model_dir, 'HighFreq_DMCNN_model(N0).pth')))
model.eval()
...
image_save_dir = os.path.join(Results_dir, 'images/DM-CNN(N0)/')
...
results.append({'Model': 'DMCNN', 'Image Type': 'RGB', 'Noise Type': 'N0', 'PSNR': avg_psnr, 'SSIM': avg_ssim})
